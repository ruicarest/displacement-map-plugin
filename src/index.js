...
var loadTextureForUnifrom = (texture, uniform, type) => {
  return new THREE.TextureLoader().load(texture, function (texture) {
    ...
    if (type == 2) {
      uniforms.u_imageSize.value = new THREE.Vector2(
        texture.image.width,
        texture.image.height
      );
    } else if (type == 1) {
      uniforms.u_maskSize.value = new THREE.Vector2(
        texture.image.width,
        texture.image.height
      );
    } else if (type == 3) {
      uniforms.u_mouseFilterSize.value = new THREE.Vector2(
        texture.image.width,
        texture.image.height
      );
    } else {
...
window.onload = function () {
  ...
  shaderGUI
    .add(effectProperties, "MaxHorizontalDisplacement", -1.0, 1.0)
    .listen();
  shaderGUI
...
function UpdateCanvasSize() {
  var scale = effectProperties.fitCanvasToImage
    ? effectProperties.imageScale
    : 1;

  if (effectProperties.fitCanvasToImage) {
    renderer.setSize(
      gameImage.image.width * scale,
      gameImage.image.height * scale
    );
  } else {
    renderer.setSize(
      effectProperties.canvasWidth,
      effectProperties.canvasHeight
    );
  }
...
function UpdateShaderParameters(elapsedTime) {
  ...
  effectProperties.MaxHorizontalDisplacement =
    currentSet.displacementX +
    ((nextSet.displacementX - currentSet.displacementX) *
      (elapsedTime - currentTimeStamp)) /
      (nextTimeStamp - currentTimeStamp);
  effectProperties.MaxVerticalDisplacement =
    currentSet.displacementY +
    ((nextSet.displacementY - currentSet.displacementY) *
      (elapsedTime - currentTimeStamp)) /
      (nextTimeStamp - currentTimeStamp);
  effectProperties.imageScale =
    currentSet.imageScale +
    ((nextSet.imageScale - currentSet.imageScale) *
      (elapsedTime - currentTimeStamp)) /
      (nextTimeStamp - currentTimeStamp);
}
